...
class FulfillmentTest(integration_test_utils.IntegrationTestBase):
  ...
  def test_fulfillment_flow(self) -> None:
    ...
    self.assertNotEmpty(checkout_with_options.fulfillment["methods"])
    method = checkout_with_options.fulfillment["methods"][0]
  ...
  def test_dynamic_fulfillment(self) -> None:
    ...
    options = us_checkout.fulfillment["methods"][0]["groups"][0]["options"]
    self.assertTrue(
    ...
    options = ca_checkout.fulfillment["methods"][0]["groups"][0]["options"]
    self.assertTrue(
  ...
  def test_known_customer_one_address(self) -> None:
    ...
    method = updated_checkout.fulfillment["methods"][0]
    self.assertIsNotNone(method.get("destinations"))
  ...
  def test_known_customer_multiple_addresses_selection(self) -> None:
    ...
    self.assertEqual(
      final_checkout.fulfillment["methods"][0]["selected_destination_id"],
      "addr_2",
    )

    # Verify selection details from the selected destination
    method = final_checkout.fulfillment["methods"][0]
    selected_dest = next(
  ...
  def test_new_user_new_address_persistence(self) -> None:
    ...
    self.assertIsNotNone(method_2.get("destinations"))
  ...
  def test_free_shipping_on_expensive_order(self) -> None:
    ...
    options = updated_checkout.fulfillment["methods"][0]["groups"][0]["options"]
