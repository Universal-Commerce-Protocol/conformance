...
def validate_version(
    ...
    version = profile.ucp.version.root
    ...
    services_val = profile.ucp.services.root
    caps_val = profile.ucp.capabilities.root
...
def _extract_capability_names(
    ...
    """Extract capability names from either schema format."""
...
def validate_capabilities(
    ...
    """Check required and optional capabilities."""
    ...
    known = REQUIRED_CAPABILITIES | OPTIONAL_CAPABILITIES
...
def validate_handlers(
    ...
    if not profile.payment or not profile.payment.handlers:
        report.add(
            CheckResult(
                "handlers.present",
                "warn",
    ...
    report.add(
        CheckResult(
            "handlers.present",
            "pass",
            f"{len(handlers)} handler(s) declared",
            "Payment Handlers",
        )
    )
    ...
    for h in handlers:
        ...
        handler_name = h.name if hasattr(h, "name") and h.name else None
...
def _extract_urls(profile: UcpDiscoveryProfile) -> list[tuple[str, str]]:
    """Extract all spec/schema URLs from the discovery profile."""
    urls: list[tuple[str, str]] = []

    # Services
    for svc_name, svc_val in profile.ucp.services.root.items():
        services = svc_val if isinstance(svc_val, list) else [svc_val]
        for idx, svc in enumerate(services):
            prefix = f"services['{svc_name}'][{idx}]"
            if svc.spec:
                urls.append((f"{prefix}.spec", str(svc.spec)))
            if svc.schema_:
                urls.append((f"{prefix}.schema", str(svc.schema_)))
            if svc.rest and svc.rest.schema_:
                urls.append(
                    (f"{prefix}.rest.schema", str(svc.rest.schema_))
                )
            if svc.mcp and svc.mcp.schema_:
                urls.append(
                    (f"{prefix}.mcp.schema", str(svc.mcp.schema_))
                )
            if svc.embedded and svc.embedded.schema_:
                urls.append(
                    (f"{prefix}.embedded.schema", str(svc.embedded.schema_))
                )
    ...
    if isinstance(caps, dict):
        for cap_name, cap_list in caps.items():
            for i, cap in enumerate(cap_list):
                ...
                if cap.spec:
                    urls.append((f"{prefix}.spec", str(cap.spec)))
                if cap.schema_:
                    urls.append((f"{prefix}.schema", str(cap.schema_)))
    else:
        for i, cap in enumerate(caps):
            name = cap.name or f"index_{i}"
...
def _find_rest_endpoint(profile: UcpDiscoveryProfile) -> str | None:
    ...
    if isinstance(shopping, list):
        for svc in shopping:
            if svc.rest and svc.rest.endpoint:
                return str(svc.rest.endpoint)
            if svc.endpoint and (
                svc.transport == "rest" or svc.transport is None
            ):
                return str(svc.endpoint)
    else:
        if shopping.rest and shopping.rest.endpoint:
            return str(shopping.rest.endpoint)
        if shopping.endpoint:
            return str(shopping.endpoint)
    return None
...
def validate_smoke(
    ...
    # Minimal checkout create payload
    payload = {
        "id": str(uuid.uuid4()),
        "currency": "USD",
        "line_items": [
